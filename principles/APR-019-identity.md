...
Its shape is fixed by [`schemas/project.schema.yaml`](../schemas/project.schema.yaml) and validated in CI ([`tools/project/check-project.ts`](../tools/project/check-project.ts)). `id`, `title`, and `owner` are mandatory (identity + accountability); the rest are optional payload.
...
- **Manifest validity** — `project.yaml` conforms to its schema; `id` is reverse-DNS; `title` / `owner` present ([`check-project.ts`](../tools/project/check-project.ts)).
...
- **Not the component-metadata contract.** [APR-014](APR-014-declare.md) governs a component's frontmatter (`metadata.core.*` / `observe.*`); node identity is a top-level attribute on *every* artifact and project identity is one manifest — neither is duplicated into the component registry.
