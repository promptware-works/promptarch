...
- [`check-digests.ts`](check-digests.ts) — verifies each [APR digest](../principles/digests/) states the same version as its source APR, and flags stale or orphan digests. Run from this directory:

  ```bash
  node check-digests.ts      # or:  npm run check:digests
  ```
...
- [`digest-generator.scope.md`](digest-generator.scope.md) — design for regenerating the digests from their source APRs. `check-digests.ts` is its first (deterministic) increment; the extractor (MVP) and the LLM condensation pass (full) follow.
